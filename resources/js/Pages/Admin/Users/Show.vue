<template>
    <admin-layout>
        ...
        <div class="flex flex-col mx-auto px-6 py-2 w-1/2">
            <div>
                <h2
                    class="text-center text-xl font-semibold capitalize underline"
                >
                    {{ user.name }}
                </h2>
                <form @submit.prevent="submit">
                    ...
                    <div>
                        ...
                        ></jet-input>
                        <jet-input-error
                            :message="form.errors.banned_until"
                            class="mt-2"
                        ></jet-input-error>
                    </div>
                    <div class="flex flex-col mt-2">
                        <div class="inline-block relative">
                            ...
                            ></jet-label>
                            <select
                                class="block capitalize border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm mt-1 w-full"
                            ...
                            >
                                <option
                                    ...
                                    :value="option.name"
                                    :selected="
                                        option.name === form.roles[0][0].name
                                    "
                                >
                                    {{ option.name }}
                                </option>
...
export default {
    ...
    created() {
        ...
        if (this.user.banned_until) {
            return this.form.banned_until.push(this.user.banned_until);
        }
    },
    methods: {
        submit() {
